...
startInterval (Interval i) = i.start

endInterval: Interval -> Maybe Date
endInterval (Interval i)= i.end
...
compare (Interval intA) (Interval intB) = 
                ...
                in Model.Util.chainCompare equal (Date.compare intA.start intB.start)
...
view interval =
    div [class "interval"] [
        div [style "float" "left"] [text "Interval:"],
        div [class "intreval-start",style "float" "left"][ Date.view (startInterval interval) ],
        div [style "float" "left"] [text " - "],
        div [class "intreval-end"][endInterval interval |> Maybe.map Date.view |> Maybe.withDefault (text "Present")],
        div [class "interval-length"][if (length interval) /= Nothing then text (fromTupleToString (Maybe.withDefault (0,0) (length interval)) ++ " years") else text ""]
